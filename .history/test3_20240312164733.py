...
def plot_one_box(x, img, color=[100, 100, 100], text_info="None",
    ...
    if velocity is not None:
        ...
        #print("c2[1],c1[1],c2[0],c1[0]",c2[1],c1[1],c2[0],c1[0])
        direction = ""
        if c2[0] > c1[0]:
            direction = "Right"
        elif  c2[0] < c1[0]:
...
def main(config):
    ...
    while not cap.end:
        ...
        if len(cap.stack) == 25:
            ...
            if yolo_preds.pred[0].shape[0]:
                ...
                print("slowfast------->:", slowfaster_preds)
        ...
        # 保存deepsort和slowfast输出到字典
        deepsort_results[cap.idx // 25] = deepsort_outputs
        slowfast_results[cap.idx // 25] = slowfaster_preds.tolist()
    ...
    # 保存slowfast输出到json文件
    with open("slowfast_output.json", "w") as f:
        json.dump(slowfast_results, f)
