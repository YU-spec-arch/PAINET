...
def save_yolopreds_tovideo(yolo_preds, id_to_ava_labels, color_map, output_video, vis=False,
                           pred_output_file="pred_trajectory_data.json", real_output_file="real_trajectory_data.json"):
    ...
    for i, (im, pred) in enumerate(zip(yolo_preds.ims, yolo_preds.pred)):
        ...
        if pred.shape[0]:
            for j, (*box, cls, trackid, vx, vy) in enumerate(pred):
                ...
                # Pass velocity information to plot_one_box function
                im = plot_one_box(box, im, color, text, (vx, vy))

                # Collect data for prediction and real trajectories
                if cls == 0:
                    pred_trajectory_data.append({
                        "Frame": i,
                        "TrackID": int(trackid),
                        "Class": yolo_preds.names[int(cls)],
                        "Box": [float(value) for value in box],  # Convert box values to float
                        "Velocity": [float(vx), float(vy)]  # Convert velocity values to float
                    })
                else:
                    real_trajectory_data.append({
                        "Frame": i,
                        "TrackID": int(trackid),
                        "AVA_Label": ava_label,
                        "Box": [float(value) for value in box]  # Convert box values to float
    ...
    # Save prediction trajectory data to JSON file
    with open(pred_output_file, "w") as f:
        json.dump(pred_trajectory_data, f, indent=4)

    # Save real trajectory data to JSON file
    with open(real_output_file, "w") as f:
        json.dump(real_trajectory_data, f, indent=4)
...
def main(config):
    ...
    # Write trajectory data to file after video processing
    pred_trajectory_data_path = "pred_trajectory_data.json"
    real_trajectory_data_path = "real_trajectory_data.json"
    print(f"Prediction trajectory data saved to {pred_trajectory_data_path}")
    print(f"Real trajectory data saved to {real_trajectory_data_path}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--input', type=str,
                        default="E:\\Computing_Power_Measurement\\Intention\\yolo_slowfast-master\\Data\\shipin5.mp4",
